...
export default function SalesReportChat() {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'Hi! I\'m your sales report assistant. I can generate sales reports for any date range. Just let me know what period you\'d like to see.',
      timestamp: new Date()
    }
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(process.env.NEXT_PUBLIC_SALES_SERVER_HOST)
      ...
      <div className="bg-gray-800/80 backdrop-blur-sm border-b border-gray-700 px-4 sm:px-6 py-3 sm:py-4 shadow-lg">
        <div className="flex items-center gap-3 max-w-5xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg sm:text-xl font-bold truncate">Sales Report Agent</h1>
            <p className="text-xs sm:text-sm text-gray-400 truncate">Generate sales reports for any date range</p>
          </div>
      ...
      <div className="px-4 sm:px-6 py-2 bg-gray-800/50 backdrop-blur-sm border-t border-gray-700/50">
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-wrap gap-2 mb-2">
            <button
              onClick={() => handleQuickAction('Show me sales report for today')}
            ...
            <button
              onClick={() => handleQuickAction('Sales report for last week')}
              className="px-3 py-1.5 bg-gray-700/50 hover:bg-gray-700 rounded-lg text-xs sm:text-sm transition-all hover:shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed border border-gray-600"
              disabled={isLoading}
            >
              <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 inline mr-1.5" />
            ...
            <button
              onClick={() => handleQuickAction('Sales report for this month')}
              className="px-3 py-1.5 bg-gray-700/50 hover:bg-gray-700 rounded-lg text-xs sm:text-sm transition-all hover:shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed border border-gray-600"
              disabled={isLoading}
            >
              <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 inline mr-1.5" />
